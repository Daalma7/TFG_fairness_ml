...
pareto_optimal_indivs = get_individual_list(pareto_optimal.dropna(), model)
for i in range(len(pareto_df)):

    print("-> Reading data from file: " + pareto_files[i])

    # Getting all individuals
    indivs = get_individual_list(pareto_df[i].dropna(), model)
...
if model == "DT":
    ...
    regex = re.compile("individuals_pareto_" + dataset + ".*_var_" + variable + ".*_model_LR.*_obj_gmean_inv__dem_fpr" + ".*_ext_.*.csv")
...
if model == "DT":
    ...
    regex = re.compile("individuals_pareto_" + dataset + ".*_var_" + variable + ".*_model_LR.*_obj_gmean_inv__dem_fpr__dem_ppv.*.csv")
    ...
    for root, dirs, files in os.walk(rootdir):
        for file in files:
            if regex.match(file):
                ...
                near['weight'] = near['weight']/near['weight'].min()    #Times bigger than the min distance. This step ends up being totally irrelevant due to the next line
...
if(len(objectives) == 4):
    ...
    # dat = pd.DataFrame({'NSGA-II₂':[0.07465], 'NSGA-II₄':[0.1125], 'SMS-EMOA':[0.4336], 'GrEA':[0.3791]}) # Not considering Ricci
    dat = pd.DataFrame({'NSGA-II₂':[0.05972], 'NSGA-II₄':[0.1248], 'SMS-EMOA':[0.3749], 'GrEA':[0.5505]}) # Considering Ricci
    sns.barplot(data=dat, ci=None)
    plt.savefig("../results/images/nondominated.png", dpi=200)
    plt.clf()
    #dat2 = pd.DataFrame({'NSGA-II₂':[2], 'NSGA-II₄':[2], 'SMS-EMOA':[12], 'GrEA':[8]}) # Not considering Ricci
    dat2 = pd.DataFrame({'NSGA-II₂':[2], 'NSGA-II₄':[4], 'SMS-EMOA':[13], 'GrEA':[11]}) # Considering Ricci
