...
for i in range(len(pareto_files)):
    ...
    pareto_df.append(pd.read_csv(pareto_files[i]).dropna())                                                                      #We're going to read that file
...
for indiv in all_indivs:                       #Now we calculate all the individuals non dominated by any other (pareto front)
    ...
    if indiv.domination_count == 0:                            #Could be done easily more efficiently, but could be interesting to see the amount of dominated individuals
...
pareto_optimal_df = pareto_optimal_df.dropna()
