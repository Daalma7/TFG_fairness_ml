...
def get_matrices(df_name, seed, model):
    df = read_data(df_name, model)
    X = df.iloc[:, :-1]
    y = df.iloc[:, -1]
    X_train, X_test, y_train, y_test = train_test_split(X, y, random_state = seed)
    if model == 'LR':               # We have to normalise after dividing between training and test in order to avoid Data Snooping
        scaler = MinMaxScaler()
...
def train_model(df_name, seed, model, **features):
    while True:     # There were some problems with parallel executions, and for that reason we include a loop to ignore EmptyDataError 
        try:
    ...
    if model == "LR":
        if features['class_weight'] is not None:
            clf = LogisticRegression(max_iter=features['max_iter'], penalty='elasticnet', tol=features['tol'], C=features['lambda'], l1_ratio=features['l1_ratio'], class_weight = {0:features['class_weight'], 1:(10-features['class_weight'])}, random_state=seed, solver='saga')
        else:
            clf = LogisticRegression(max_iter=features['max_iter'], penalty='elasticnet', tol=features['tol'], C=features['lambda'], l1_ratio=features['l1_ratio'], class_weight = features['class_weight'], random_state=seed, solver='saga')
